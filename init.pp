...
fall2sh() {
    ...
    echo "--> $*"
    echo "Reboot with initramfs=shell(noprobe) to further debug the 
issue."
...
probe_kms() {
    for device in /sys/bus/pci/devices/*/boot_vga; do
        ...
        grep -q 1 $device && modprobe -bq `cat 
${device%boot_vga}modalias`
    done
    if [ ! -c /dev/fb0 ]; then
        echo "options uvesafb scroll=ywrap mtrr=0 nocrtc=1 
mode_option=1024x768-32" > /etc/modprobe.d/uvesafb.conf
        modprobe -bq uvesafb
    fi
}
...
mount_rootfs() {
    FS_TYPE=`disktype $ROOT_DEVICE | grep KERNELMODULE | awk '{print 
$2}'`
            ...
            then
                ...
                then
                    ...
                    then
                        ...
                        cp -af /newroot/mnt/cdrom/* 
/newroot/mnt/cdromtemp/
...
mount_cdroot() {
    ...
    # Loop type squashfs
    mount -t squashfs -o loop,ro /newroot/mnt/cdrom/${LOOPBACKFILE} 
/newroot/mnt/livecd
    ...
    # FIXME: the device list is taken from udev, we can't rely on sys 
entries since pluggable means different
    #        in kernel world. Suggestions that do not include this kind 
of regexp mania are welcome

    # for userspace applications
    REAL_ROOT_TYPE=`echo "${ROOT_DEVICE}" | sed -e 
's/^\/newroot\/dev\///' | grep -qE '^sr[0-9]*|^hd[a-z]|^pcd[0-9]|^xvd*' 
&& echo "optical" || echo "harddisk"`
...
# First parse config file, then cmdline to allow overwriting internal 
config
if [ -f "$INITRAMFSCONF" ]
then
    #. $INITRAMFSCONF
...
# This stops /lib/udev/rules.d/65-md-incremental.rules from medling with 
mdraid sets.
...
test -x /bin/plymouth && /bin/plymouth update-root-fs 
--new-root-dir=/newroot
